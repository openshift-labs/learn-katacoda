...
ssh root@host01 'for i in {1..200}; do oc new-app --template=rhdm70-full-persistent -p APPLICATION_NAME="loan-demo" -p IMAGE_STREAM_NAMESPACE="openshift" -p KIE_ADMIN_USER="developer" -p KIE_ADMIN_PWD="developer" -p KIE_SERVER_CONTROLLER_USER="developer" -p KIE_SERVER_CONTROLLER_PWD="developer" -p KIE_SERVER_USER="developer" -p KIE_SERVER_PWD="developer" -p MAVEN_REPO_USERNAME="developer" -p MAVEN_REPO_PASSWORD="developer" -e JAVA_OPTS_APPEND=-Derrai.bus.enable_sse_support=false -n loan-demo && break || sleep 2; done'
