...
ssh root@host01 'for i in {1..200}; do oc new-app --template=rhdm70-full-persistent -p APPLICATION_NAME="loan-demo" -p IMAGE_STREAM_NAMESPACE="openshift" -p KIE_ADMIN_USER="dmAdmin" -p KIE_ADMIN_PWD="redhatdm1!" -p KIE_SERVER_CONTROLLER_USER="kieserver" -p KIE_SERVER_CONTROLLER_PWD="kieserver1!" -p KIE_SERVER_USER="kieserver" -p KIE_SERVER_PWD="kieserver1!" -p MAVEN_REPO_USERNAME="dmAdmin" -p MAVEN_REPO_PASSWORD="redhatdm1!" -e JAVA_OPTS_APPEND=-Derrai.bus.enable_sse_support=false -n loan-demo && break || sleep 2; done'
